...
{
  const char *filename;
  unsigned int xnuVersion=0;

  // get filename, xnuVersion from args

  if (argc != 3) { printf("usage: replay <xnuVersion> <filename>\n"); exit(2); }
  ...
  // replay messages from file

  netstatClient->runRecording((char *)filename, xnuVersion);
